...
function customSet(arr) {
  ...
  for (let value of set) { // Перебираємо множину за допомогою 'for of'. Цикл 'for of' дозволяє пройтись по всіх елементах колекції.
...
function clearSet(set) {
  if (set.size > 0) { // Використання властивості size для перевірки розміру множини, чи більше вона нуля
    set.clear(); 
    return "Множину очищено."; // Якщо так використання методу clear для очищення множини та повертаємо Множину очищено.
  } else { 
    return "Множина вже порожня."; // Якщо ні повертаємо Множина вже порожня.
  }
}
...
function filterAndAdd(mySet, arr) {
  ...
  for (const element of mySet) { 
    if (typeof element === "number") { 
      mySet.delete(element); // Якщо елемент є числом, видаляємо його з множини за допомогою методу delete.
    }
  }
  for (const element of arr) { // Тепер множина не містить числових значень. Перебираємо масив та додаємо кожний елемент до множини
    mySet.add(element); 
...
function setToArray(mySet) {
  ...
  const arrayFromSet = [...mySet]; // Конвертуємо множину в масив за допомогою деструктурізації.
  const stringElementsArray = arrayFromSet.filter((item) => typeof item === "string"); // Використовуємо метод filter для створення нового масиву, що містить лише рядкові елементи.
  const sortedArray = stringElementsArray.sort(); // Використовуємо метод sort для сортування рядкових елементів в алфавітному порядку.
  return sortedArray; // Повертаємо оброблений масив.
}
...
function removeDuplicatesInPlace(arr) {
  ...
  if (uniqueSet.has(arr[i])) { 
    ...
    } else { // Додаємо унікальний елемент до множини
      uniqueSet.add(arr[i]); 
    }
  }
...
function getIntersection(arr1, arr2) {
  const set1 = new Set(arr1); // Створення множин з двох масивів
  const set2 = new Set(arr2); 
  const intersectionSet = new Set(); // Створення множини intersectionSet
  for (const element of set1) { // Перебір елементів першої множини за допомогою оператору for of
    if (set2.has(element)) { // Перевірка, чи елемент є спільним у другій множині
...
function iterateSet(set) {
 ...
 for (const key of set) {  // Використовуємо for...of для кожного ключа з ітератора keys
  console.log(key); 
 } // Виведення ключа у консоль
  // Використовуємо методу values для отримання ітератора значень
  for (const value of set.values()) { // Використовуємо for...of для кожного значення з ітератора values
    console.log(value); 
  } // Виведення значення у консоль
  for (const entry of set.entries()) { // Використовуємо методу entries для отримання ітератора записів
